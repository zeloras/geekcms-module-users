<form action="{{ route('admin.users.save', ['page' => (isset($user) && !empty($user->id)) ? $user->id : null]) }}" method="POST">
    @csrf
                            ...
                            <div class="card-block display-table text-left">
                                @foreach ($permission as $line)
                                    <div class="checkbox-toggle">
                                        <input type="checkbox" @if (in_array($line['name'], $init_permissions)) checked="checked" @endif class="user-permissions__input" name="user_permission[{{$line['name']}}]" value="{{$line['name']}}" id="{{$module}}_{{$line['name']}}">
